"""Instruction fine-tuning for the ApnaAI model.
...
Key principles:
  - Data quality > quantity: curated instruction data
  - Streaming: data is NOT fully downloaded
  - Fine-tuning only: start from pre-trained checkpoint
  - English only
...
def main():
    ...
    for it in range(MAX_ITERS + 1):
        ...
        if it % EVAL_INTERVAL == 0:
            ...
            model.train()
            dt = time.time() - t0
            print(f"iter {it:5d} | val loss {val_loss:.4f} | lr {lr:.2e} | {dt/60:.1f} min", flush=True)
        ...
        if it % LOG_INTERVAL == 0 and it % EVAL_INTERVAL != 0:
            dt = time.time() - t0
            print(f"iter {it:5d} | train loss {loss.item():.4f} | lr {lr:.2e} | {dt/60:.1f} min", flush=True)
